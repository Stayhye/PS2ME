...
jint rmfsRead(jint identifier, void *buffer, jint size)
{
  ...
  for (i = 0; i < MAX_DATABLOCK_NUMBER; i++) {
	...
	while (readSize < size) {
	  ...
	  /* ps2-fix (j2me-ps2): each linked block must be copied to buffer at
	   * the running offset (bytes already read), not always to buffer[0].
	   * The stock code computed tempPos but passed `buffer`, so every block
	   * of a multi-block file overwrote the first block's data -> the caller
	   * got a corrupt file. That garbage later flowed into malloc/free-sized
	   * structures and detonated in newlib _free_r. tempPos is taken BEFORE
	   * advancing readSize so it points just past the previous blocks. */
	  tempPos = (void *) ((jint) buffer + readSize);

	  readSize += rmfsDataBlockHdrArray[blockIndex].dataSize;

	  if (readSize >= size) {
	    rmfsReadBlock(blockIndex, tempPos,
			  size - (readSize -
