...
// "music=0|1", "controllayout=0|1", "setup=0|1"), persisted on the memory card (falling
// back to the boot directory) via Ps2MemCard. See Settings.hpp.
...
Settings::Settings()
    : showRecent_(true), defaultSort_(0), debugMode_(false), fpsCounter_(false),
      bgmEnabled_(true), controlLayout_(0), layoutChosen_(false), loaded_(false) {}
...
void Settings::load() {
    ...
    while (i < total) {
        ...
        } else if (strncmp(line, "setup=", 6) == 0 && len >= 7) {
            layoutChosen_ = (line[6] != '0');
        }
    }
}
...
void Settings::setLayoutChosen(bool on) {
    layoutChosen_ = on;
    save();
}
